...
function slideEffect(){
    ...
    function initEvent(){
        for(const item of dotBtn){ // 모든 dot버튼에 이벤트 적용하여 dotOver 함수실행
    ...
    function visualSlide() { // 비주얼 슬라이드 실행함수
        ...
        if (isSlide == false) { //  슬라이드 하지 않을때만 슬라이드 실행

            isSlide = true; // 슬라이드를 진행하면 isSlide 상태변수를 true로 지정하여 슬라이드하는 동안 다음 슬라이드 명령이 중복되지 않게 설정
            if(dotOverNum ==0){
                gsap.set(slideBanner,{backgroundImage:"linear-gradient(0deg, rgba(0, 0, 0, 0.6), " +
     ...
     function autoPlay(){ // 5초마다 slideCount 함수실행하여 자동 슬라이드 실행
